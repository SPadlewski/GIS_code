...
Correlation_all <- df %>%
  st_drop_geometry()%>%
  dplyr::select(!c(area_name,area_code,JPT_NAZWA_,pm25_dif,pm25_02_08,model3resids,gwr.modelresids,pm25)) %>%
  correlate()

#visualise the correlation matrix
rplot(Correlation_all,print_cor = T)
...
I_LsGminy_Global_fur <- df%>%
  pull(fur) %>%
  as.vector()%>%
  moran.test(., Gminy.queens_weight)

I_LsGminy_Global_fur
