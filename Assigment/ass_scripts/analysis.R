...
transformed <-preProcess(weather [,c(2:4)],method = c("range"))
transformed <-  predict(transformed,weather[,c(2:4)]) %>% 
  mutate(area_code=weather$area_code)
#Load all pollutants 
...
AQI <- pollutants %>% 
  mutate_at(vars(c(2:21)),
             funs(con2aqi(pollutant="pm10",con=.))) %>% 
  mutate_at(vars(c(22:39)),
            funs(con2aqi(pollutant="pm25",con=.))) %>% 
  mutate_at(vars(c(40:56)),
            funs(con2aqi(pollutant="co",con=.))) %>% 
  mutate_at(vars(c(57:76)),
            funs(con2aqi(pollutant="no2",con=.)))%>% 
  mutate_at(vars(c(77:96)),
            funs(con2aqi(pollutant="nox",con=.)))%>% 
  mutate_at(vars(c(97:116)),
            funs(con2aqi(pollutant="so2",con=.)))%>% 
  mutate_at(vars(c(117:135)),
            funs(con2aqi(pollutant="pm25",con=.)))%>% 
  mutate_at(vars(c(136:155)),
            funs(con2aqi(pollutant="o3",con=.)))
...
pm10_dif <- pollutants%>% 
  ...
  mutate(pm10_08_19= round(rowMeans(.[c(11:21)], na.rm = TRUE),1)) %>%
  mutate(pm10_00_08= round(rowMeans(.[c(2:10)], na.rm = TRUE),1)) %>% 
  mutate(pm10= pm10_08_19-pm10_00_08) %>% 
  select(area_code,pm10)

pm25_dif <- pollutants%>% 
  dplyr::select(c(1,22:39))%>%
  mutate(across(.cols = 2:19, as.numeric)) %>% 
  mutate(pm25_08_19= round(rowMeans(.[c(9:19)], na.rm = TRUE),1)) %>%
  mutate(pm25_02_08= round(rowMeans(.[c(2:8)], na.rm = TRUE),1)) %>% 
  mutate(pm25= pm25_02_08-pm25_08_19) %>% 
  select(area_code,pm25)

pm10_dif_19 <- pollutants%>% 
  dplyr::select(c(1:21))%>%
  mutate(across(.cols = 2:21, as.numeric)) %>% 
  mutate(pm10_00_08= round(rowMeans(.[c(2:10)], na.rm = TRUE),1)) %>% 
  mutate(pm10= pm10_00_08-.[21] ) %>% 
  select(area_code,pm10)

pm10_dif_t <-preProcess(pm10_dif[,2],method = c("range"))
pm10_dif_t <-  predict(pm10_dif_t,pm10_dif[,2]) %>% 
  mutate(area_code=pm10_dif$area_code)
...
furnaces_total <- furnaces_final %>% 
  summarise(all_removed)

sum(furnaces_final$all_removed)
...
df_2 <- gminy_LP %>% 
  ...
  left_join(.,transformed,by="area_code") %>% 
...
#log1p independent
independent_loghist <- df_2_tidy%>%
  filter(All_variables=="tmin")%>% 
  ggplot(., aes(x=log(val,base=9))) + 
  geom_histogram(aes(x = val, y = ..density..),color="black", fill="white")+
  geom_density(colour="red", size=1, adjust=1)+
  facet_wrap(~All_variables, scales = 'free')
independent_loghist
...
furnaces_map <- tm_shape(df_2, bbbox = ls_bb) + 
...
tm_shape(df_2) +
  tm_polygons(col="pm10_Iz",
              ...
              palette="RdGy",
              midpoint=NA,
              title="Local Moran's Iz, Pm10 in London")
...
df_2<- df_2 %>%
  ...
  plyr::mutate(pm25_LocGiz = as.numeric(G_LsGminy_Global_PM25))
...
#model2
colnames(df_2)
model2 <- lm(pm25 ~ log1p(furnaces_per_1000)+log1p(density_km2)+log1p(roads_density_km2)+
               log1p(altitude_m_masl)+log1p(urban_landuse)+log1p(total_tourists_per_1000) +
               log10(tmax)+log10(tmin)+log10(prec)+,
             data = df_2)
tidy(model2)
summary(model2)
...
#model3
colnames(df_2)
model3 <- lm(pm25 ~ log1p(furnaces_per_1000)+log1p(density_km2)+log1p(roads_density_km2)+
               altitude_m_masl+log(population_08_19)+log(total_area_m2)  ,
             data = df_2)
tidy(model3)
summary(model3)

# quick plot
ggplot(df_2, aes(y=pm25,x=log1p(furnaces_per_1000))) + 
  geom_point()
...
pm25_model3_queen <- lagsarlm(pm25 ~ log1p(furnaces_per_1000)+log1p(density_km2)+log1p(roads_density_km2)+
...
pm25_model3_knn4 <- lagsarlm(pm25 ~ log1p(furnaces_per_1000)+ log1p(density_km2)+log1p(roads_density_km2)+
                               log1p(altitude_m_masl)+log1p(urban_landuse) , 
                             data = df_2, 
...
GWRbandwidth <- gwr.sel(pm25 ~ log1p(furnaces_per_1000)+ log1p(density_km2)+log1p(roads_density_km2)+
                          log1p(altitude_m_masl)+log1p(urban_landuse), 
                        data = df_2SP, 
                        coords=coordsR_adSP,
                        adapt=T)
...
#run the gwr model
gwr.model = gwr(pm25 ~ log1p(furnaces_per_1000)+ log1p(density_km2)+log1p(roads_density_km2)+
                  log1p(altitude_m_masl)+log1p(urban_landuse), 
                data = df_2SP, 
                coords=coordsR_adSP , 
                adapt=GWRbandwidth, 
