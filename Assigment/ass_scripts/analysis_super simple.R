...
pm10 <- pollutants%>% 
  ...
  mutate(pm10_08_19= round(rowMeans(.[c(11:21)], na.rm = TRUE),1)) %>%
  mutate(pm10_00_08= round(rowMeans(.[c(2:10)], na.rm = TRUE),1)) %>% 
  select(area_code,pm10_00_08,pm10_08_19)
...
pm10_ls_mean <-pollutants_ls_mean %>% 
...
# H0 - intruduced policy is not influancing the air qual
...
pm10_hist_08 <- df_2_tidy%>%
  filter(All_variables=="pm10_00_08")%>%
  ggplot(., aes(x=val)) + 
  geom_histogram(aes(x = val, y = ..density..),color="black", fill="white")+
  geom_density(colour="red", size=1, adjust=1)
pm10_hist_08

pm10_hist_19 <- df_2_tidy%>%
  filter(All_variables=="pm10_08_19")%>%
  ggplot(., aes(x=val)) + 
  geom_histogram(aes(x = val, y = ..density..),color="black", fill="white")+
  geom_density(colour="red", size=1, adjust=1)
pm10_hist_19
...
breaks1<- c(0,20,30,35,40,45,50,60)

pm10_map_19 <- tm_shape(df_2, bbbox = ls_bb) + 
   tm_polygons(col='pm10_08_19',breaks=breaks1,
                palette="YlOrRd",alpha=0.5)
pm10_map_19

pm10_map_08 <- tm_shape(df_2, bbbox = ls_bb) + 
  tm_polygons(col='pm10_00_08',breaks=breaks1,
              palette="YlOrRd",alpha=0.5)
pm10_map_08
...
##Testing
#Moran’s I test
I_LsGminy_Global_PM10_19 <- df_2 %>%
  pull(pm10) %>%
  as.vector()%>%
  moran.test(., LsGminy.lw)

I_LsGminy_Global_PM10_19
#Geary’s C

C_LsGminy_Global_PM10_19 <- df_2 %>%
  pull(pm10_08_19) %>%
  as.vector()%>%
  geary.test(., LsGminy.lw)

C_LsGminy_Global_PM10_19
...
#model2
colnames(df_2)
model2 <- lm(pm10 ~ log1p(furnaces_per_1000)+log1p(density_km2)+log1p(roads_density_km2)+
...
#model3
colnames(df_2)
model3 <- lm(pm10 ~ log1p(furnaces_per_1000)+log1p(density_km2)+log1p(roads_density_km2)+
...
# quick plot
ggplot(df_2, aes(y=pm10,x=log1p(furnaces_per_1000))) + 
  geom_point()
